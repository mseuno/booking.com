...
<title><bk:printProperty key="admin_city.title"/></title>
</head>
...
<div class="wrapper">
	...
	<div class="apllication">
		 ...
		 <div class="apllication" id="content" >
				<h1><bk:printProperty key="admin_city.page_name"/></h1>
				<form action="admincity" method="get">
				<table>
				<tr>
					<td>
						<input type="text" name="search_name" value="${search_result }"/> 
								<input type="submit" name="search" value="<bk:printProperty key="admin_city.submit_city_search"/>"/>
					</td>
					<td>
					<!-- SELECT LANGUAGE -->
							...
							<c:forEach items = "${languages}" var = "list" >
									<c:choose>
											<c:when test="${list.id != AdminCity_page_language}">	
					...
					<td>
													<!-- SELECT COUNTRY -->
								<select name="search_country_id" id="country-selector" onchange="this.form.submit()">
									<c:choose>
											<c:when test="${'search_all' != AdminCity_country_id.toString()}">	
													<option   value="search_all"><bk:printProperty key="admin_city.url_all"/></option>				
											</c:when>
											<c:otherwise>
													<<option selected="selected" value="search_all"><bk:printProperty key="admin_city.url_all"/></option>
											</c:otherwise>
									</c:choose>	
									<c:forEach items="${countries }" var="list">
										<c:choose>
											<c:when test="${list.id != AdminCity_country_id}">
												<option value="${list.id}">${list.name }</option>
											</c:when>
											<c:otherwise>
													<option selected="selected" value="${list.id}">${list.name }</option>
							...
							<td>
								...
								<select onchange="this.form.submit()" name="search_region_id" id="region-selector">
									<c:choose>
											<c:when test="${'search_all' != AdminCity_region_id.toString()}">	
									...
									<c:forEach items="${regions }" var="list">
										<c:choose>
											<c:when test="${list.id != AdminCity_region_id}">
												<option value="${list.id}">${list.name }</option>
											</c:when>
											<c:otherwise>
													<option selected="selected" value="${list.id}">${list.name }</option>
							...
							<td>
								<input type="submit" name="edit_create" value="<bk:printProperty key="admin_city.submit_city_create"/>" class="btn-small btn-success">
							</td>
				</tr>
				</table>
