...
<title><bk:printProperty key="admin.edit_city.title"/></title>
</head>
	...
	<div id="body" style="margin-left: 10px ">
		...
		<div id="menu" style="width: 200px; float: left;">
	 	 ...
	 	 <div id="content" align="center">
				<h1><bk:printProperty key="admin.edit_city.page_name"/></h1>

						<form action="admincity" method="post">
												<!-- SELECT COUNTRY -->
								...
								<c:forEach items="${languages }" var="list">
									...
									<c:choose>
										...
										<c:otherwise>
											...
											<c:forEach items="${city }" var="inner">
												<c:if test="${inner.localeId == list.id}">
													...
													<td>
														...
														<input type="hidden" value="${inner.id}" name="city_id">
													</td>
							...
							<c:choose>
								<c:when test="${empty city || not empty for_create}">
									<input type="submit" value="<bk:printProperty key="admin.edit_city.submit_create"/>" name="create">
									<input type="hidden" value="<bk:printProperty key="admin.edit_city.submit_create"/>" name="update_page">
								</c:when>
								<c:otherwise>
									<input type="submit" value="<bk:printProperty key="admin.edit_city.submit_update"/>" name="update">
									<input type="hidden" value="<bk:printProperty key="admin.edit_city.submit_update"/>" name="update_page">
								</c:otherwise>
							</c:choose>
							<c:if test="${not empty city && empty for_create }">
								<input type="submit" value="<bk:printProperty key="admin.edit_city.submit_delete "/>" name="delete">
							</c:if>
					</form>
			</div>
		</div>
